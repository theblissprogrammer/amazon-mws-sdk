...
class SellersNetworkStore(val httpService: HTTPServiceType): SellersStore {
    ...
    override fun fetch(request: SellerModels.CurrentRequest) : Result<Seller> {

        val response = httpService.get(request.marketplace.amazonBaseURL + "/sp",
                parameters = mapOf(Pair("seller", request.id)))

        return if (response.value == null || !response.isSuccess) {
    ...
    private fun fetchSellerRank(request: SellerModels.CurrentRequest, sellerName: String?): String? {
