 ...
 */
class ReportRequestListXmlParser {
    // We don't use namespaces
    private val ns: String? = null

    @Throws(XmlPullParserException::class, IOException::class)
    fun parse(input: String): List<RequestReport>? {
        input.byteInputStream().use { inputStream ->
            ...
            return parser.findChildTag("GetReportRequestListResult") {
    ...
    private fun readRequest(parser: XmlPullParser): RequestReport? {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            when (name) {
                ...
                "StartDate" -> startDate = parser.readDate(name)
                "EndDate" -> endDate = parser.readDate(name)
